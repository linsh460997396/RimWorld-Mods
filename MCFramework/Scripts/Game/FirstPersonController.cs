...
namespace MCFramework
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        [SerializeField] private float m_GravityMultiplier = 0f; //重力倍数
        ...
        public KeyCode backKey = KeyCode.K;
        ...
        void HandleMovement()
        {
            ...
            if (Input.GetKey(backKey)) move -= transform.forward;
            if (Input.GetKey(leftKey)) move -= transform.right;
            if (Input.GetKey(rightKey)) move += transform.right;

            transform.Translate(speed * Time.deltaTime * move.normalized, Space.World);
        }
        ...
        void Init()
        {
            ...
            //创建新的独立相机
            m_Camera = CreatePerspectiveCamera("FirstPersonCharacter", default);
            //m_Camera.tag = "MainCamera";
            m_Camera.gameObject.transform.parent = transform;
            ...
            m_Camera.gameObject.AddComponent<FlareLayer>();
            ...
            //    //自动质心、张量
            //    gameObject.GetComponent<Rigidbody>().automaticCenterOfMass = true;
            //    gameObject.GetComponent<Rigidbody>().automaticInertiaTensor = true;
            //    //运动学
            ...
            //Debug.Log("在CPEngine物体上创建1个Unity预制的胶囊体"); 
            GameObject capsule = GameObject.CreatePrimitive(PrimitiveType.Capsule); //携带完整组件
            capsule.transform.parent = gameObject.transform;

            //配置FOVKick曲线
            if (m_FovKick.IncreaseCurve == null)
            {
        ...
        private void Start()
        {
            ...
            m_OriginalCameraPosition = m_Camera.transform.localPosition;//获取摄像机的本地坐标
        ...
        /// <returns></returns>
        public static Camera CreatePerspectiveCamera(string name = "FirstPersonCharacter", Vector3 position = default, Vector3 lookAt = default, float fieldOfView = 60f, float nearClipPlane = 0.3f, float farClipPlane = 1000f)
        {
            // 创建新的游戏对象
            ...
            //camera.rect = new Rect(0, 0, 1, 1); // X,Y为左下角起点，W,H为宽高比例
            //// 渲染路径使用图形渲染
            //camera.renderingPath = RenderingPath.UsePlayerSettings;
            //camera.targetTexture = null; // 默认渲染到屏幕
            //// 遮挡剔除
