...
namespace MetalMaxSystem.UnityEditor
{
    /// <summary>
    /// 纹理分析器(用于场景图片扫描分析,打印纹理编号文本,输出特征图).
    /// 仅UnityEditor支持本类中图片处理方法(无法参与打包),其他如Winform可请使用MMCore的TextureAnalyzerNet类.
    /// </summary>
    public class TextureAnalyzer : MonoBehaviour
    {
        ...
        public static void SpriteMerger(List<Sprite> sprites, string fileName, int maxSpritesPerRow, string savePathFrontSP, bool fromBottomLeft = true)
        {
            ...
            /// <summary>
            /// 若是精灵编辑器制作的带Meta数据图片,而非直接用函数切割的精灵
        ...
        /// <param name="similarity">相似度(0到1之间的浮点数)</param>
        /// <returns>若两个颜色相似则返回true,否则返回false</returns>
        ...
        public static void SliceTextureAndSetSpriteIDTask(string folderPath, string searchPattern, float similarity, string savePathFrontSP, string savePathFrontIndex, int handleCountMax = 10, int pixelX = 16, int pixelY = 16, int xCount = 8, bool fromBottomLeft = true)
        {
            ...
            {
                fileCount++;
                //若图片过多,下面动作由多线程进行处理(存在多个Task修改共用资源情况下,需要给修改动作上锁或用跨线程安全的数据类型)
                tasks.Add(
                    Task.Run(() =>
                    {
            ...
            //等待所有任务完成
            //await Task.WhenAll(tasks.ToArray()); //等待所有任务完成后,支持返回一个包含所有任务结果的数组(若任务有返回值的话)
...
// 按引用传递
// 若你想要 Lambda 表达式能够修改原始变量,你需要使用 ref 关键字来按引用传递参数.这适用于值类型和引用类型.
