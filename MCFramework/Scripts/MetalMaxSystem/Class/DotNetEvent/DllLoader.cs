...
namespace MetalMaxSystem
{
    ...
    public static class DllLoader
    {
        ...
        /// </summary>
        /// <param name="lpLibFileName">要加载的DLL文件的路径.可以是绝对路径或相对路径,若是相对路径则相对于当前进程的工作目录</param>
...
// 对于返回值,若不显式指定[return: MarshalAs(UnmanagedType.Bool)],编译器会尝试根据方法的签名和上下文来推断返回值的类型
// 在大多数情况下,若外部方法的返回类型是布尔值并且使用的是标准的Windows API函数,那么编译器通常能够正确地推断出返回值的类型并自动进行封送
...
// SetLastError=true:这个参数告诉.NET若FreeLibrary函数执行失败,它应通过调用Marshal.GetLastWin32Error()方法来获取错误代码,对错误处理和调试非常有用.
// extern:表示该方法是在外部实现的
