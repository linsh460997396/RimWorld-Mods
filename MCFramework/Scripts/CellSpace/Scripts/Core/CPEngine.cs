...
namespace CellSpace
{
    ...
    public class CPEngine : MonoBehaviour
    {
        ...
        // 若是Unity编辑器模式进行开发,可通过菜单、场景中组件对象来编辑这些带小写L的等价物,并在Awake方法中,将结果初始化赋值给实际游戏变量.
        ...
        /// <summary>
        /// 发送镜头注视事件(若为true, CameraEventsSender组件将把事件发送到主摄像机视场中心指着的单元).
        /// </summary>
        public static bool sendCameraLookEvents;
        /// <summary>
        /// 发送鼠标指针事件(若为true, CameraEventsSender组件将将把事件发送到当前鼠标光标指着的单元).
        /// </summary>
        ...
        /// 若将MultiplayerTrackPosition字段设置为true,则表示该物体的位置将在网络上进行同步,且每个客户端都将跟踪该物体的轨道位置.
        /// 若将MultiplayerTrackPosition字段设置为false,则表示该物体的位置不会在网络上进行同步,而客户端将不会跟踪其轨道位置.
        ...
        /// <summary>
        /// 3D顶部纸片模式.启用后所有单元(体素块)只显示上面,屏蔽底面和侧面渲染.
        ...
        /// <summary>
        /// (从GUI界面、配置文件输入)发送鼠标指针事件(若为true, CameraEventsSender组件将将把事件发送到当前鼠标光标指着的单元)
        /// </summary>
        ...
        public static void Init()
        {
            ...
            SaveVoxelData = false;
                ...
                //若团块生成距离<1则被置为0(不再生成),MC玩法默认8即可,单一空间玩法可设置0(表示四周的邻居团块均不创建)
                ...
                if (QualitySettings.antiAliasing > 0)
                {
                    Debug.LogWarning("CellSpace: Anti-aliasing is enabled. This may cause seam lines to appear between blocks. If you see lines between blocks, try disabling anti-aliasing, switching to deferred rendering path, or adding some texture padding in the engine settings." +
                        "启用了抗锯齿,这可能导致在块之间出现接缝线！若你看到块之间的线条,试着禁用抗锯齿,切换到延迟渲染路径,或者在引擎设置中添加一些纹理填充.");
                }
        ...
        public static void Tick(Vector3 pos = default)
        {
            //刷地图前保证前置核心组件已经初始化完成
            if (!CPEngine.initialized || !CellChunkManager.Initialized) { return; }
            //若多人模式已启用但连接尚未建立则不加载团块
            ...
            if (pos == default)
            {
                ...
                pos = CPEngine.ChunkCreationIndicator.position;
            }
            TickFunc(pos);
        }
        /// <summary>
        ...
        public static void BlocksPreFill(int num)
        {
            ...
            blocks[0].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[1].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[2].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[3].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[4].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[5].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[6].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[7].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[8].SetActive(false);
            blocks[8].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[9].SetActive(false);
            blocks[9].transform.parent = CellSpacePrefab.group.transform;
            ...
            blocks[10].SetActive(false);
            blocks[10].transform.parent = CellSpacePrefab.group.transform;
        ...
        public static void CreatePrefab(string vName, ushort cellID, ushort subMeshIndex, bool createGameObject = true, GameObject gameObject = null, bool vCustomMesh = false, Mesh vMesh = null, MeshRotation vRotation = MeshRotation.none, Transparency vTransparency = Transparency.solid, ColliderType vColliderType = ColliderType.none, bool vCustomSides = false, Vector2 vector0 = default, Vector2 vector1 = default, Vector2 vector2 = default, Vector2 vector3 = default, Vector2 vector4 = default, Vector2 vector5 = default)
        {
            ...
            {//若强制创建游戏物体
                ...
                {//参数未填写已有游戏物体
                    ...
                    CPEngine.prefabOPs[cellID].gameObject.SetActive(false);
                    CPEngine.prefabOPs[cellID].gameObject.transform.parent = CellSpacePrefab.group.transform;//收纳到CellSpacePrefab.group下
                }
                else
                {
        ...
        public static string LoadTextFile(string path)
        {
            ...
            {
                Debug.LogWarning($"读取失败: {e.Message}");
                return null;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// 从文件中读取当前活动世界的种子,或者若没有找到种子文件则随机生成一个新的种子,并将其存储在Engine.WorldSeed变量中.
        /// </summary>
        ...
        public static GameObject GetCellGameObject(ushort cellId)
        {
            try
            {
                //若体素ID达到ushort数据类型的最大值65535那么归零
        ...
        public static Cell GetCellType(ushort cellID)
        {
            try
            {
                //若体素ID达到ushort数据类型的最大值65535,那么归零(防止从负数开始)
                if (cellID == ushort.MaxValue) cellID = 0;
        ...
        /// “ignoreTransparent”为true时光线投射将穿透透明或半透明的单元,若没有击中则返回null.注意:若碰撞体生成被禁用,此函数将不起作用,另在2D模式下Z=0但默认插入一个立方体,只是纹理在Up面改为Forward
        /// </summary>
        ...
        { // a raycast which returns the index of the hit cell and the gameobject of the hit chunk
            ...
            {
                //若从hit碰撞体组件对象里能获取到团块或团块扩展组件(这里因为Collider是继承Component的,所以可直接使用父类的GetComponent方法获取当前游戏物体对象身上的其他兄弟组件)
                ...
                { // check if we're actually hitting a chunk.检查我们是否真的击中了团块
                    ...
                    { // if we hit a mesh container instead of a chunk.若我们击中的是网状容器而不是团块(判断依据是网状容器拥有大块扩展组件),注意网格容器是团块大小的,虽是团块的子对象但它不是单元)
                    ...
                    { // punch through transparent voxels by raycasting again when a transparent cell is hit.当一个透明单元被击中时,再次通过光线投射穿透透明单元
                        ...
                        //若命中的单元类型的VTransparency属性!=实心,说明是透明或半透明
        ...
        /// “ignoreTransparent”为true时光线投射将穿透透明或半透明的单元.若没有击中任何块,则返回null.注意:若碰撞体生成被禁用,此函数将不起作用.
        /// </summary>
        ...
        //块编辑器中若勾选Custom Mesh则启用默认材质,只有不勾选Custom Mesh时可以自定义材质,也就是可以选择图片纹理偏移
        ...
        public static Vector2 GetTextureOffset(ushort cellID, Facing facing)
        {
            ...
            { // in case there are no textures defined, return a default texture.以防万一若没有定义纹理,则返回默认的(0, 0)
            ...
            { // if this cell isn'transform using custom side textures, return the Up texture.若这个单元没有使用自定义单面纹理,直接返回单元朝上的那面纹理点(6个面共用)
                return textureArray[0];
            }
            //将面向这个枚举类型转整型(上下左右前后默认对应0~5),如面向所代表的数值超过纹理点阵长度-1,将被判定为没有定义纹理(自定义6面却忘了填完它们)
            else if ((int)facing > textureArray.Length - 1)
            { // if we're asking for a texture that's not defined, grab the last defined texture instead.若我们请求了一个没有定义的纹理,则抓取最后定义的纹理点(剩下的面会采用它代表的精灵的纹理)
